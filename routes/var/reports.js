// SUCCESS
const CARD_CHECKED_OUT = 'Card checked out.';
const PAYMENT_REPORT_ADDED = 'Payment report added.';

// FAILED
const PAYMENT_REPORT_FAILED = 'Payment report failed.';
const MEMBER_UPDATE_FAILED = 'Member update failed.';
const ERROR_WHILE_SEARCHING = 'Error while searching. Please contact the developer.';
const CHECKOUT_ERROR = 'Checkout Error. Please contact the developer.';
const ACTIVATION_ERROR = 'Activation Error. Please contact the developer.';

// ALERT
const CARD_ACTIVATED = 'Card has been activated successfully.';
const CARD_ADDED = 'Card has been added successfully';
const CARD_NOT_EXIST = 'Card does NOT EXIST';
const CARD_NOT_ACTIVE = 'Card is NOT ACTIVE';
const CARD_NOT_MEMBER = 'Card is NOT A MEMBER CARD';
const CARD_BELONGS_TO_OTHER = 'Card BELONGS TO OTHER Member';

module.exports = {
  CARD_CHECKED_OUT,
  PAYMENT_REPORT_ADDED,
  PAYMENT_REPORT_FAILED,
  MEMBER_UPDATE_FAILED,
  ERROR_WHILE_SEARCHING,
  CHECKOUT_ERROR,
  ACTIVATION_ERROR,
  CARD_ACTIVATED,
  CARD_ADDED,
  CARD_NOT_EXIST,
  CARD_NOT_ACTIVE,
  CARD_NOT_MEMBER,
  CARD_BELONGS_TO_OTHER,
};
